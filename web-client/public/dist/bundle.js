...
(() => {
  ...
  var resultVideo = document.querySelector("#resultVideo");
  ...
  uploadForm.addEventListener("submit", async (event) => {
    ...
    if (json.mimetype == "image") {
      const url = `data:image/jpeg;base64,${import_buffer.Buffer.from(json.data).toString(
        "base64"
      )}`;
    ...
    if (json.mimetype == "video") {
      const url = `data:video/mp4;base64,${import_buffer.Buffer.from(json.data).toString(
        "base64"
      )}`;
  ...
  uploadImageBtn.addEventListener("click", async () => {
    ...
    if (json.mimetype == "video") {
      resultVideo.src = `data:video/avi;base64,${import_buffer.Buffer.from(json.data).toString(
        "base64"
      )}`;
    }
  });
